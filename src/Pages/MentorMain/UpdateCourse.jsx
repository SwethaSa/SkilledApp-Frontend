...
export default function UpdateCourse() {
  ...
  const toastOptions = {
    ...
    iconTheme: { primary: "#fff", secondary: "#F0EBFF" },
  };
    ...
    async function fetchCourse() {
      try {
        ...
        if (res.ok) {
          ...
          setModules(data.modules || [{ moduleTitle: "", content: "" }]);
        } else {
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        toast.error(data.message || "Update failed", toastOptions);
      }
    } catch (err) {
      toast.error(err.message, toastOptions);
    }
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (res.ok) {
        toast.success("Course deleted", toastOptions);
        navigate("/mentor-dashboard");
      } else {
        toast.error(data.message || "Delete failed", toastOptions);
      }
    } catch (err) {
      toast.error(err.message, toastOptions);
    }
  ...
  if (loading) {
    return <div className="create-course-container">Loading...</div>;
  }

  return (
    <div className="create-course-container">
      <Toaster position="top-center" reverseOrder={false} />
      <h2 className="section-title">Update Course</h2>
      <form className="course-form" onSubmit={handleUpdate}>
